...
export default function FaceCamera() {
  ...
  const [notDetected, setNotDetected] = useState(true);
      ...
      .then((res2) => {
        ...
        if (res2.data.faces && res2.data.faces.length > 0) {
          ...
          if (
            ...
            setVaccineInfo({
              isVaccinated: true,
              info: vaccineData.find(
                (x) => x.faceId == res2.data.results[0].face_token
              ),
            });
      ...
      {capturedImage && (
        <>
          ...
          {faceBox && faceBox.left && (
            <>
              {!notDetected && vaccineInfo.info && (
                <Text
                  style={{
                    color: isVaccinated ? "green" : "red",
                    fontSize: 20,
                    position: "absolute",
                    bottom: 20,
                    left: 20,
                ...
                >
                  Name: {vaccineInfo.info.fullName}
                  {"\n"}
                  Vaccine Status:{vaccineInfo.info.status}
                </Text>
              )}
